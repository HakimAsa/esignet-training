...
[`render.yaml`](render.yaml) is a [Blueprint](https://render.com/docs/blueprint-spec)
defining the web service, built from [`prod.Dockerfile`](prod.Dockerfile). In
the Render dashboard: **New → Blueprint**, point it at this repo.

Postgres is **not** provisioned by this Blueprint — Render's managed Postgres
has no free plan (it prompts for payment info even via Blueprint). Use a
free-tier external provider instead (e.g. [Neon](https://neon.tech) or
[Supabase](https://supabase.com)) and set `DATABASE_URL` to its connection
string when Render prompts for it. If you'd rather use Render's own Postgres
anyway, add back a `databases:` block (see git history of this file, or
Render's Blueprint docs) and change `DATABASE_URL` to a `fromDatabase` reference.

`prod.Dockerfile` is a separate file from the root `Dockerfile` used for local
dev/testing: its `CMD` runs `prisma migrate deploy` before starting the server
on every boot (safe/idempotent — it only applies pending migrations), which
the local Dockerfile intentionally doesn't do.

A few env vars are marked `sync: false` in `render.yaml` — Render prompts for
these on first apply rather than filling them in automatically:

- `DATABASE_URL` — your external Postgres connection string
- `CORS_ORIGIN` — your deployed frontend's origin
- `ESIGNET_CLIENT_ID`, `ESIGNET_PRIVATE_KEY`, `ESIGNET_KEY_ID` — see the
  key-pair section above
- `ESIGNET_REDIRECT_URI` — this is chicken-and-egg: Render only assigns your
  service's `https://<name>.onrender.com` URL on first deploy, so deploy once,
  then set `ESIGNET_REDIRECT_URI` to `https://<your-service>.onrender.com/auth/callback`
  (or your custom domain) and register that exact URL with MOSIP before login will work

`ESIGNET_SESSION_SECRET` is auto-generated by Render (`generateValue: true`) —
