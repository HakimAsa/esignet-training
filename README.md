...
defining a web service (built from [`prod.Dockerfile`](prod.Dockerfile)) plus a
managed Postgres instance. In the Render dashboard: **New → Blueprint**, point
it at this repo, and Render provisions both.
...
no need to set it manually. `DATABASE_URL` is wired automatically from the
Blueprint's Postgres instance.
